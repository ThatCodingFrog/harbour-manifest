...
async function updateGames() {
    // In GitHub Actions, process.cwd() is the root of your repo
    ...
    async function processPorts(portObject, currentSubDir = "") {
        for (const [key, value] of Object.entries(portObject)) {
            if (typeof value === 'string') {
                // This builds: [RepoRoot]/ports/[N64 or GC]/[soh.json]
                const fullPath = path.join(repoRoot, "ports", currentSubDir, value);
                
                if (!fs.existsSync(fullPath)) {
                    console.error(`⚠️ FILE MISSING: Expected ${key} at ${fullPath}`);
                    // Debug: Show what IS in that directory
                    const dirToSearch = path.dirname(fullPath);
                    if (fs.existsSync(dirToSearch)) {
                        console.log(`Contents of ${dirToSearch}:`, fs.readdirSync(dirToSearch));
                    }
                    continue; 
                }

                await updateSingleGame(key, fullPath);
            } else if (typeof value === 'object' && value !== null) {
                // Recurse into subfolders like N64, GC
                await processPorts(value, path.join(currentSubDir, key));
            }
        }
    }
    ...
    if (manifest.ports) {
        ...
        console.error("❌ CRITICAL: 'ports' key missing in manifest.json");
    }
}
...
async function updateSingleGame(id, fullPath) {
    try {
        ...
        if (!res.ok) {
            console.error(`❌ GitHub API Error for ${id}: ${res.status}`);
            return;
        }
        ...
        if (release.tag_name !== gameData.version) {
            ...
            release.assets.forEach(asset => {
                for (const [platform, regex] of Object.entries(PLATFORM_PATTERNS)) {
                    if (regex.test(asset.name)) foundAssets[platform] = asset.browser_download_url;
                }
            });
